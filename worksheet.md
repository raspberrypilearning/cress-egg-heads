...
Go to `Menu` > `Programming` > `Python 3 (IDLE)` to open up a Python shell. Then click on `File` > `New File`. Click `File` > `Save` and call your file `cresseggheads.py`
...
Have a look in your home directory. There's only one image!
...
To do this you can use a variable to count the number of images taken, and use string formatting to use that number in the image name.
...
The line before the loop begins sets the `image_number` variable to be 0. Within the loop `image_number` is used to create a string called `image_name`. The name is made up of the strings `image` and `.jpg` but in between those strings is the image number, padded so it is 4 digits long (`{0:04d}`)

You could alter `{0:04d}` to `{0:02d}` for instance, if you only wanted filename numbers that were two digits long.

Now save and run your program to see what happens. Do you see lots of images being created in your directory?
...
The first thing to do is to check the size of the images your PiCamera has been taking. The PiCamera image size will default to your screen resolution, unless told otherwise, so image sizes may vary.

To check the size of an image, open a terminal (`ctrl+alt+t`) and type the following command:
...
Next, you can see how much space you have left on your microSD card, by typing
...
You're only interested in the top number in the `Avail` column. Here it is showing 3.6 gigabytes of space left.

So multiply 3.6 by 1,000,000 tells you there are 3,600,000 kilobytes of space left on the microSD card.
With each image taking up 4024 kilobytes; 3,600,000 divided by 4024 is approximately 895 images.

If there were to be one image captured every hour, that would mean that 895 ÷ 24 which is around 37 days of photographs, which in this case is more than enough to capture some cress seeds growing.
...
Once the image sequence has been captured you need to stitch the images together to form a film file that can be played back. There are a set of tools called avconv (audio video convert) that you can use for this.
