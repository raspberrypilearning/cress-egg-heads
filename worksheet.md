...
## Setting up for the real timelapse
...
With each image taking up 4024 kilobytes; 3,600,000 divided by 4024 is approxiamtely 895 images.

If there were to be one image captured evey hour, that would mean that 895 ÷ 24 which is around 37 days of photographs, which in this case is more than enough to capture some cress seeds growing.
...
You can now run your script and leave it to record the images. Once your cress has finished growing (after a few days) you can stop the script with `ctrl+c`.
...
Once the image sequence has been cpatured you need to stitch the 60 or so images together to form a film file that can be played back. There are a set of tools called avconv (audio video convert) that pyou can use for this.

```bash
avconv -r 10 -i image%04d.jpg -r 10 -vcodec libx264 -crf 20 -g 15 timelapse.mp4
```

The `-r` specifies the video frame rate. Here we're using 10 frames a second; this is adequate for a time-lapse film. It's used twice to avoid avconv dropping similar looking frames. The `-i` is the input filename; notice the `%04d`. This is becuase your images have names with 4 digit numbers in them.
